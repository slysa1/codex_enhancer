...
- [install_enhancer.bat](install_enhancer.bat): Windows launcher that opens the Qt GUI installer, with browser fallback, through PowerShell-backed Python discovery.
...
- [scripts/launch_enhancer_gui.ps1](scripts/launch_enhancer_gui.ps1): PowerShell launcher helper that resolves the real Python runtime behind PowerShell-visible commands or standard per-user install locations before starting the GUI.
