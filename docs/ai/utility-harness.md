...
- The installer never installs those dependencies by default; it runs `python -m pip install -r requirements-codex.txt` only when an operator explicitly selects the dependency-install option.
...
## Audit Use
During a repository improvement audit, harness output can support findings only when it is tied back to inspected repo files and explicit commands. Use `tools/ai/audit_inputs.py` to inventory candidate evidence files, roadmap targets, and recorded validation commands without executing them. Use `tools/ai/run_checks.py --list` before running anything so prose-extracted commands remain visible and inert by default.
...
## Non-Goals
- no default dependency installation
