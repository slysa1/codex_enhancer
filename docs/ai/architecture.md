...
9. [install_enhancer.bat](../../install_enhancer.bat), [scripts/launch_enhancer_gui.ps1](../../scripts/launch_enhancer_gui.ps1), [scripts/install_enhancer_qt_gui.py](../../scripts/install_enhancer_qt_gui.py), and [scripts/install_enhancer_web_gui.py](../../scripts/install_enhancer_web_gui.py): Windows-first installer entrypoint for PowerShell-backed real Python runtime discovery from commands or standard per-user install locations, optional PyQt6/PySide6 standalone UI, no-dependency browser fallback, manual repo path entry, overwrite review, pack management, upgrade/reconcile, and guided install or managed-output refresh flow. [scripts/install_enhancer_gui.py](../../scripts/install_enhancer_gui.py) remains the legacy Tkinter wrapper and shared preview/completion helper surface.
...
18. [tests/](../../tests/): regression protection for the validator, installer core, command facade, Qt/browser GUI helpers, legacy GUI-facing helpers, stack-pack loader, Spec Kit bridge detector, and Utility Harness resolver.
