...
- The manifest records lifecycle state, pack-selection mode, managed section ids, managed output ownership, selected packs, selected workflows, per-pack and per-workflow evidence, and resolved Spec Kit bridge state under `[integrations.spec_kit]`.
...
## Change Workflow Packs After Upgrade
Use `--manage-workflows` after the target reports as current:
...
- Confirm `selected_workflows` agrees with the `selected = true` flags in `[[detected_workflows]]`.
...
- Do not change selected workflows during upgrade; use `--manage-workflows` as a separate, reviewable step.
